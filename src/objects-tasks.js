...
function makeWord(lettersObject) {
  const wordLength = Object.values(lettersObject).flat().length;
  const wordArr = new Array(wordLength);
  Object.keys(lettersObject).forEach((key) => {
    const letterPositions = lettersObject[key];
    letterPositions.forEach((position) => {
      wordArr[position] = key;
    });
  });
  return wordArr.join('');
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((obj) => {
    const objKey = keySelector(obj);
    const objValue = valueSelector(obj);

    if (multiMap.has(objKey)) {
      const val = multiMap.get(objKey);
      val.push(objValue);
      multiMap.set(objKey, val);
    } else {
      multiMap.set(objKey, [objValue]);
    }
  });
  return multiMap;
}
